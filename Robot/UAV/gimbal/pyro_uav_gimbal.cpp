...
status_t uav_gimbal_t::_init()
{
    ...
    //             9.0f,50,30,2);
    gimbal_ctx.cfg.pid_ctx.auto_pitch_position_pid = new pid_t(25.0f,0.008f,0.0f,0.8f,
                8.0f,50,30,4);
    gimbal_ctx.cfg.pid_ctx.auto_pitch_speed_pid = new pid_t(1.5f,0.075f,0.002f,1.0f,
                7.0f,45,20,4);
...
//
//     // 3. Pitch 轴计算
//     float pitch_speed_ff = ctx->pitch_td.x2 * 0.8f;
//
...
//
//     ctx->data.gravity_compensate = -0.762f + term1 + term2;
//     if (abs(ctx->data.gravity_compensate) > 1.3f) ctx->data.gravity_compensate = 1.3f;
//
...
void uav_gimbal_t::auto_aim_gimbal_control(gimbal_ctx_t *ctx)
{
    ...
    constexpr float R_BASE = 800.0f;       // 基础收敛速度（静态/小移动时用）
    constexpr float R_MAX_EXT = 1500.0f;   // 最大额外增量 (总计最高 4000)
    constexpr float ERROR_FREE = 0.02f;
    constexpr float ERROR_MAX = 0.35f;
    // 计算线性映射系数 (0.0 到 1.0)
    float r_ratio = (yaw_error - ERROR_FREE) / (ERROR_MAX - ERROR_FREE);
    r_ratio = std::clamp(r_ratio, 0.0f, 1.0f);
    // 更新当前 TD 的 r
    ctx->yaw_td.r = R_BASE + (R_MAX_EXT * r_ratio);
    // 执行 TD 计算
    td_calculate(&ctx->yaw_td, ctx->data._target_yaw_angle);

    //pitch 动态调整r 提高响应速度 无斜坡过度 直接增大 以响应小陀螺
    float pitch_error = ctx->data._target_pitch_angle - ctx->data._current_imu_pitch_angle;
    // --- 动态 TD 策略  不用斜坡了 直接跳变提高响应速度
    if (fabsf(pitch_error) > 0.15f)
    {
        ctx->pitch_td.r = 1500.0f;
    }
    else
    {
        ctx->pitch_td.r = 900.0f;
    }
    td_calculate(&ctx->pitch_td, ctx->data._target_pitch_angle);
    ...
    // 2. 最终前馈 = 自瞄 + TD 先拿yaw试试
    float combined_ff = (kalman_yaw_v * 0.7f) + (ctx->yaw_td.x2 * 0.3f * error_factor);
